...
var current_model_name = "cards-krczm/1";
...
async function switchModel() {
  ...
  var prechosen_images = document.getElementById("prechosen_images");

  var prechosen_images = prechosen_images.children;

  for (var i = 0; i < prechosen_images.length; i++) {
    prechosen_images[i].src =
      available_models[current_model_name]["imageGrid"][i];
  }
...
function webcamInference() {
  ...
  if (
    ...
    navigator.mediaDevices
      ...
      .catch(function (err) {
        // replace with img
        setImageState(CAMERA_ACCESS_URL);
      });
  }
}
  ...
  if (
    ...
    reader.onload = function (event) {
      ...
      img.onload = function () {
        ...
        modelWorkerId.then((modelWorkerId) => {
          inferEngine
            .infer(modelWorkerId, new inferencejs.CVImage(img))
            .then((predictions) => {
              ctx.drawImage(
                img,
                sx,
